...
def download_model():
    ...
    Ham nay duoc goi mot lan khi module duoc import. AWS credentials co the
    den tu IAM role tren EC2 hoac bien moi truong trong systemd service.
    """
    os.makedirs(os.path.dirname(MODEL_PATH), exist_ok=True)
    s3 = boto3.client("s3")
    s3.download_file(AWS_BUCKET, S3_MODEL_KEY, MODEL_PATH)
    print(f"Model da duoc tai xuong tu s3://{AWS_BUCKET}/{S3_MODEL_KEY}.")
